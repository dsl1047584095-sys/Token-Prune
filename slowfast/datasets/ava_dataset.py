...
class Ava(torch.utils.data.Dataset):
    ...
    def _load_data(self, cfg):
        ...
        filtered_video_idx_to_name = []
        filtered_boxes_and_labels = []

        # 遍历所有视频名称，跳过没有对应 boxes_and_labels 的视频
        for i, video_name in enumerate(self._video_idx_to_name):
            if video_name in boxes_and_labels:
                filtered_image_paths.append(self._image_paths[i])
                filtered_video_idx_to_name.append(video_name)
                filtered_boxes_and_labels.append(boxes_and_labels[video_name])
            else:
                # 简单跳过，没有必要记录日志
                continue

        # 更新图像路径和视频名称列表
        self._image_paths = filtered_image_paths
        self._video_idx_to_name = filtered_video_idx_to_name
        boxes_and_labels = filtered_boxes_and_labels
