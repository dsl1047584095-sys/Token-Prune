...
def draw_predictions(frame, boxes, preds, persons, label_path, threshold):
    """
    在图像上绘制检测结果
    Args:
        frame: 输入图像
    ...
    # 绘制每个检测结果
    for box, pred, person_box, person_score in zip(
        boxes, preds, person_boxes, person_scores
    ):
        ...
        # 获取最高置信度的动作
        action_score, action_idx = torch.max(pred, dim=0)
        if action_score < threshold:
            continue
            
        # 添加标签文本
        label = f"{labels[action_idx]}: {action_score:.2f}"
        cv2.putText(
            result,
            label,
            (x1, y1 - 10),
