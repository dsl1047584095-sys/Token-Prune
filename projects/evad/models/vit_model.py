...
class KTPAttention(Attention):
    ...
    def forward_part1(self, x, last_idx=None, ws=None, global_rho=None):
        B, N, C = x.shape
        keep_rate = global_rho if global_rho is not None else self.keep_rate
        qkv_bias = None
        if self.q_bias is not None:
            qkv_bias = torch.cat((self.q_bias, torch.zeros_like(self.v_bias, requires_grad=False), self.v_bias))
        # qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
    ...
    def forward(self, x, last_idx=None, ws=None, global_rho=None):
        B, N, C = x.shape
        keep_rate = global_rho if global_rho is not None else self.keep_rate
        if self.keep_rate < 1:
        ...
        if self.keep_rate < 1:
            num_keep_tokens = math.ceil(keep_rate * N)
...
class KTPBlock(nn.Module):
    ...
    def forward_part1(self, x, last_idx, window_size,global_rho=None):
        if self.gamma_1 is None:
            attn, idx = self.attn(self.norm1(x), last_idx, window_size,global_rho=global_rho)
    ...
    def forward(self, x, last_idx, window_size,global_rho=None):
        # attn
        tmp, idx = self.forward_part1(x, last_idx, window_size, global_rho=global_rho)
        x = x + tmp
